...
class DemoSchedule:
    ...
    def add_task(
        ...
        *,
        ...
        if task_type in ("TT_Mile", "TT_FinMile"):
            ...
            ef = (
                es
                if task_type == "TT_Mile"
                else datetime.combine(next_workday(start_day), time(16))
            )
        dur_hr = dur_days * DAY_HOURS
        remain_hr = (
            remain_days * DAY_HOURS
        ...
        lf = (
            ef
            if float_hours == 0
            else datetime.combine(add_workdays(ef.date(), int(float_hours / DAY_HOURS)), time(16))
        )
        ls = (
            es
            if float_hours == 0
            else datetime.combine(add_workdays(es.date(), int(float_hours / DAY_HOURS)), time(8))
        )
        row: dict[str, object] = {
            "task_id": tid,
            "proj_id": proj_id,
            ...
            else (50 if status == "TK_Active" else 0),
            "rev_fdbk_flag": False,
            "lock_plan_flag": False,
            "auto_compute_act_flag": True,
            ...
            "remain_drtn_hr_cnt": remain_hr,
            "target_drtn_hr_cnt": dur_hr,
            "act_work_qty": 0,
            "remain_work_qty": 0,
            "target_work_qty": 0,
            "target_start_date": es,
            "target_end_date": ef,
            "create_user": "admin",
            "update_user": "admin",
        ...
        if status == "TK_Complete":
            ...
            row.update(
                {
                    "act_start_date": es,
                    "early_start_date": DATA_DATE,
                    "early_end_date": ef,
                    "late_start_date": DATA_DATE,
                    "late_end_date": lf,
                }
            )
        else:
    ...
    def link(
        self, pred_code: str, succ_code: str, pred_type: str = "PR_FS", lag_hr: float = 0.0
    ) -> None:
        ...
        self.b.add(
            ...
            {
                "task_pred_id": pid,
                "task_id": self.task_id_of(succ_code),
                "pred_task_id": self.task_id_of(pred_code),
                "proj_id": 1,
                "pred_proj_id": 1,
                "pred_type": pred_type,
                "lag_hr_cnt": lag_hr,
    ...
    def assign(
        ...
        act_qty = (
            qty_hr if status == "TK_Complete" else (qty_hr / 2 if status == "TK_Active" else 0.0)
        )
        ...
        self.b.add(
            ...
            TASKRSRC_FIELDS,
            {
                ...
                if t["target_drtn_hr_cnt"]
                else 0,
                "act_reg_qty": act_qty,
                "act_ot_qty": 0,
                "ot_factor": 1.5,
                "cost_per_qty": rate,
                "target_cost": qty_hr * rate,
                "act_reg_cost": act_qty * rate,
                "act_ot_cost": 0,
    ...
    def _projects(self) -> None:
        self.b.add(
            ...
            PROJECT_FIELDS,
            {
                ...
                "def_duration_type": "DT_FixedDrtn",
                "task_code_prefix": "A",
                ...
                "allow_neg_act_flag": False,
                "use_project_baseline_flag": True,
    ...
    def _activities(self) -> None:
        ...
        add("A1010", "Design Criteria", 101, 5, status="TK_Complete", start_day=date(2024, 1, 2))
        add(
            "A1020", "Preliminary Design", 101, 10, status="TK_Complete", start_day=date(2024, 1, 9)
        )
        add(
            "A1030",
            "Detailed Design",
        ...
        add("A2000", "Issue RFQ", 102, 3, status="TK_Complete", start_day=date(2024, 1, 23))
        add(
            "A2010",
            "Evaluate Bids",
            102,
        ...
        add(
            ...
            5,
            status="TK_NotStart",
            start_day=date(2024, 2, 22),
            driving=True,
            cstr_type="CS_MSO",
            cstr_date=datetime(2024, 2, 22, 8),
        )
        add(
            "A3010",
            "Foundations",
        ...
        add(
            ...
            3,
            status="TK_NotStart",
            start_day=date(2024, 4, 4),
            driving=True,
        )  # 2d lag after A3040
        add(
            "A3060",
            "Long Duration Buffer",
            103,
            25,
            status="TK_NotStart",
            start_day=date(2024, 2, 22),
    ...
    def _resources(self) -> None:
        ...
        for rid, name, short, typ, cal, qph in rows:
            self.b.add(
                ...
                RSRC_FIELDS,
                {
                    ...
                    "ot_flag": False,
                    "curr_id": 1,
                    "unit_id": 1 if typ != "RT_Mat" else 2,
                    "rsrc_type": typ,
                    "load_tasks_flag": True,
                    "level_flag": False,
        ...
        for code, rid, qty, rate, typ in [
            ("A1010", 301, 40, 120, "RT_Labor"),
            ("A1020", 301, 80, 120, "RT_Labor"),
            ("A1030", 301, 120, 120, "RT_Labor"),
            ("A1040", 301, 40, 120, "RT_Labor"),
            ("A3000", 302, 160, 65, "RT_Labor"),
            ("A3010", 302, 320, 65, "RT_Labor"),
            ("A3020", 302, 256, 65, "RT_Labor"),
            ("A3020", 303, 64, 250, "RT_Equip"),
            ("A3030", 302, 256, 65, "RT_Labor"),
            ("A3040", 302, 160, 65, "RT_Labor"),
            ("A3050", 302, 96, 65, "RT_Labor"),
            ("A3010", 304, 500, 150, "RT_Mat"),
        ]:
            self.assign(code, rid, qty, rate, typ, role_id=50 if typ == "RT_Labor" else None)
    ...
    def _codes_udfs(self) -> None:
        self.b.add(
            ...
            {
                "actv_code_type_id": 602,
                "actv_short_len": 10,
                "seq_num": 2,
                "actv_code_type": "Responsibility",
                "proj_id": 1,
        ...
        self.b.add(
            "TASKACTV", ["task_id", "actv_code_type_id", "actv_code_id", "proj_id"], *taskactv
        )
        self.b.add(
            "UDFTYPE",
            [
            ...
            ],
            {
                "udf_type_id": 701,
                "table_name": "TASK",
                "udf_type_name": "bid_package",
                "udf_type_label": "Bid Package",
            ...
            },
            {
                "udf_type_id": 702,
                "table_name": "TASK",
                "udf_type_name": "risk_score",
                "udf_type_label": "Risk Score",
        ...
        self.b.add(
            ...
            {
                "udf_type_id": 701,
                "fk_id": self.task_id_of("A3010"),
                "proj_id": 1,
                "udf_text": "BP-CIVIL-01",
            },
            {
                "udf_type_id": 702,
                "fk_id": self.task_id_of("A3020"),
                "proj_id": 1,
                "udf_number": 7.5,
            },
        )
    ...
    def _misc(self) -> None:
        ...
        self.b.add(
            ...
            ],
            {
                "acct_id": 901,
                "acct_seq_num": 1,
                "acct_name": "Direct Labor",
                "acct_short_name": "DL",
            },
            {"acct_id": 902, "acct_seq_num": 2, "acct_name": "Materials", "acct_short_name": "MAT"},
        )
        self.b.add(
            "PROJCOST",
            [
            ...
            ],
            {
                ...
                "acct_id": 902,
                "proj_id": 1,
                "task_id": self.task_id_of("A3010"),
                "cost_per_qty": 1,
                "cost_qty": 25000,
                "cost_load_type": "CL_Uniform",
                "target_cost": 25000,
                "act_cost": 0,
                "remain_cost": 25000,
                "cost_name": "Rebar Package",
                "auto_compute_act_flag": True,
                "cost_type": "Materials",
            },
            {
...
def _assemble(d: DemoSchedule) -> str:
    ...
    d.b.add(
        ...
        {"obs_id": 101, "parent_obs_id": 100, "seq_num": 2, "obs_name": "Capital Projects"},
    )
    d.b.add(
        "UMEASURE",
        ["unit_id", "unit_abbrev", "unit_name", "seq_num"],
    ...
    d.b.add(
        ...
        CALENDAR_FIELDS,
        {
            ...
            "year_hr_cnt": 2000,
            "rsrc_private": False,
            "last_chng_date": datetime(2024, 1, 1, 9),
            "clndr_data": CLNDR_DATA_5D,
        },
        {
            "clndr_id": 11,
            "default_flag": False,
            "clndr_name": "7-Day Calendar",
            "clndr_type": "CA_Base",
            "day_hr_cnt": 8,
            "week_hr_cnt": 56,
            "month_hr_cnt": 240,
        ...
        task_rows.append(
            {
                ...
                "complete_pct_type": "CP_Drtn",
                "task_code": f"T{i:06d}",
                "task_name": f"Task {i} chain {chain}",
